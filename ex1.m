...
function pushbutton1_Callback(hObject, eventdata, handles)
    ...
    b = zeros(1,10*D+1); % 10=N correspond au nombre d'echo souhait�
...
wavplay(yb, Fs); % Lecture du signal
    ...
    afft = abs(fft(x));
    bfft = abs(fft(yb)); % Transform�e de Fourier
...
n = length(b)-1;
f = 0 : handles.Fe/n : handles.Fe;
bfft = abs(fft(b)); % Transform�e de Fourier
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    a = 1; % d�nominateur 
    t= 1000000;
    wavplay(handles.x,handles.Fe); % Lecture du signal modul�
